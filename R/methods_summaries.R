...
#' Generates a summary table of outcomes from a Bayesian model
#'
#' @param fit The fitted model object, containing 'stan_fit'.
#' @param digits The number of decimal places to round the results.
#'
#' @return A tibble with key outcomes: Time Ratio, Odds Ratio,
#'         and long-term survival rates for each arm.
#' @importFrom rstan extract
#' @importFrom tibble tibble
#' @importFrom stats quantile median
#' @export
#'
outcomes <- function(fit, digits = 2) {
  # 1. Extract posterior samples
  posterior <- rstan::extract(fit$stan_fit)

  if (!"beta_cure_intercept" %in% names(posterior)) {
    stop("Parameter 'beta_cure_intercept' was not found in the model fit.")
  }

  # 2. Calculate derived quantities from the draws
  tr_draws <- exp(posterior$beta_surv_arm)
  or_draws <- exp(posterior$beta_cure_arm)
  ...
  # 3. Helper function to summarize draws using the MEDIAN for the point estimate
  summarize_draws <- function(draws, multiplier = 1) {
    # Use median for the point estimate, which is robust to skew
    point_estimate <- median(draws) * multiplier
    # Quantiles for the credible interval remain the same
    ci <- quantile(draws, probs = c(0.025, 0.975)) * multiplier
  ...
  summary_table <- tibble::tibble(
    ...
    `Result (95% CI)` = c(
      summarize_draws(tr_draws),
      summarize_draws(or_draws),
