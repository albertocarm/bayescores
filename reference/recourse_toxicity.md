...
from the RECOURSE randomized clinical trial (trifluridine/tipiracil in
mCRC), including both any‑grade (G1–4) and severe (G3–4) adverse events.
Patient‑reported QoL scores are stored in the `qol` column. The dataset
provides the number of patients per trial arm. Please refer to the
accompanying documentation or vignette for details on how QoL scores are
